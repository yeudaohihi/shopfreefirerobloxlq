          ...
          <tbody>
            @foreach ($category->groups as $item)
              <tr>
                ...
                <td>{{ $item->priority }}</td>
                <td>
                  <a href="{{ route('admin.accountsv2.groups.edit', ['id' => $category->id, 'gid' => $item->id]) }}" class="shadow btn btn-primary btn-xs sharp me-1"><i class="fa fa-edit"></i> sửa</a>
                  <a href="{{ route('admin.accountsv2.items', ['id' => $item->id]) }}" class="shadow btn btn-info btn-xs sharp me-1"><i class="fa fa-eye"></i> Xem</a>
