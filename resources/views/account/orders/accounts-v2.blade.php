    ...
    @foreach ($accounts as $account)
      <div class="grid grid-cols-3 items-center rounded-lg bg-transparent bg-cover text-white" style="background-image: url(/images/all-img/widget-bg-3.png)">
        ...
        <div class="flex flex-col justify-between whitespace-nowrap text-truncate col-span-2 p-2 text-[16px] font-bold">
          <div>{{ __t('Mã Số:') }} <span class="copy text-[#F4CE14]" data-clipboard-text="{{ $account->code }}" onclick="onClickToCode(this)">#{{ $account->code }}</span></div>
          <div>{{ __t('Mã Đơn:') }} <span class="text-[#E1AA74] copy" data-clipboard-text="{{ $account->buyer_code }}">{{ $account->buyer_code }} / {{ Helper::formatCurrency($account->buyer_paym) }}</span></div>
          <a href="{{ route('account.orders.accounts-v2', ['code' => $account->buyer_code]) }}">{{ __t('Tài Khoản:') }} <span class="text-[#40F8FF]">{{ $account->username }}</span></a>
          <div>{{ __t('Ngày Mua:') }} <span class="text-[#80B3FF]">{{ $account->buyer_date }}</span></div>
        </div>
      </div>
    @endforeach
  </div>
