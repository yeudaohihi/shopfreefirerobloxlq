...
const buyItem = async (item) => {
  ...
  try {
    const { data: result } = await axios.post('/api/stores/accounts-v2/' + item.code + '/buy')
  ...
  <section>
    ...
    <div v-if="products?.length !== 0 && !loading">
      <a-row v-if="!loading" :gutter="18">
        <a-col :xs="24" :md="8" :lg="6" v-for="item in products" :key="item.id" class="mb-2 mt-2 cursor-pointer">
          <a-badge-ribbon color="red" :text="`-${item.discount}%`" :class="{ hidden: item.discount <= 0 }" placement="start">
            <a-badge-ribbon color="pink" :text="`MS: ${item.code}`" placement="end">
              <div style="background: transparent" class="border border-primary rounded-lg p-[1px]">
                ...
                <div class="text-center text-lg font-bold bg-primary text-white" style="margin-top:-6px">{{ item.name }}</div>
                <div class="p-3">
                  ...
                  <div class="text-center grid grid-cols-2 gap-3">
                    <div class="col-span-2">
                      <div class="border border-red-500 rounded-lg p-1 font-bold">
                        {{ __t('Còn lại') }} <span class="text-green-600">{{ item.amount }}</span> nick
                      </div>
                    </div>
                    ...
                    <a-tooltip :title="item.name">
                      <button class="btn btn-sm btn-primary w-full" @click="buyItem(item)">
                        <i class="fas fa-shopping-cart me-2"></i><span>{{ __t('Mua Ngay') }}</span>
                      </button>
                    </a-tooltip>
                  </div>
                </div>
